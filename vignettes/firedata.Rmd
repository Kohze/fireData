...
fireData provides an R interface to common operations in selected Google Firebase services, enabling you to:
...
The package focuses on common REST operations for these four services. It does
not cover every Firebase product or every advanced endpoint; see
[Firebase feature support](https://github.com/Kohze/fireData/blob/master/inst/FIREBASE-SUPPORT.md).
...
The wizard configures the current session by default. Saving requires
`save = TRUE` and an explicit file path; the parent directory must exist.

```{r wizard}
library(fireData)
if (interactive()) {
  firebase_config_wizard()
  # Optional: save a configuration file in the temporary directory
